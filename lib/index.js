...
import { deepSeekAccount, kimiCodeAccount, zaiAccount } from './accounts.js';
...
function providerView(route, tokens) {
  ...
  const subscription = ['zai-coding-cn', 'zai', 'kimi-coding'].includes(providerId);
  return {
...
export async function collectToday(ctx, now = Date.now()) {
  ...
  const [deepseek, zai, kimi] = await Promise.all([
    providers.some(x => x.providerId === 'deepseek-official') ? deepSeekAccount(credentials) : null,
    providers.some(x => x.providerId === 'zai-coding-cn' || x.providerId === 'zai') ? zaiAccount(credentials) : null,
    providers.some(x => x.providerId === 'kimi-coding') ? kimiCodeAccount(credentials) : null,
  ]);
  return { ok: true, day, updatedAt: Date.now(), providers, accounts: { deepseek, zai, kimi }, activity: activityFromDaily(daily, now) };
}
